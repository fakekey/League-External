...
def on_hit_noon_quiver(me, target):
    if target.HasUnitTags("Unit_Champion"):
        return 0.0
    return 20.0


def on_hit_runaan_hurricane(me, target):
    if me.IsRanged:
        return 30.0
    return 0.0
...
def on_hit_blade_of_the_ruined_king(me, target):
    dmg = (
        target.health * 12.0 / 100.0 if not me.IsRanged else target.health * 9.0 / 100.0
    )
    if dmg > 60.0 and not target.HasUnitTags("Unit_Champion"):
        return 60.0
    return dmg
...
def is_last_hitable(me, enemy):
    hit_dmg = dmg_hit_on_armor(
        me, enemy, (me.basicAtk + me.bonusAtk + get_onhit_physical(me, enemy))
    ) + dmg_hit_on_magic_res(me, enemy, get_onhit_magical(me, enemy))
...
def get_lasthitable(me, minions: list, range=0.0):
    ...
    for minion in minions:
        ...
        if not minion.IsHurtable or (
            minion.position.distance_squared(me.position)
            >= (range + minion.info.gameplayRadius / 2) ** 2
        ):
            continue
...
def predict_pos(game, target, cumTime: float):
    ...
    if len(target.AIManager.targetPath) <= 1:
        ...
        dir = (
            target.AIManager.targetPath[1]
            .sub(target.AIManager.targetPath[0])
            .normalize()
        )
...
def circle_on_line(A, B, C, R):
    ...
    x_diff = B.x - A.x
    y_diff = B.y - A.y
    LAB = cmath.sqrt(x_diff**2 + y_diff**2)
    ...
    x_diff1 = Ex - C.x
    y_diff1 = Ey - C.y
    LEC = cmath.sqrt(x_diff1**2 + y_diff1**2)
    ...
    if oType == "champion":
        for champ in game.champs:
            if (
                champ.IsEnemyTo(me)
                and champ.isAlive
                and not champ.objIndex == target.objIndex
            ):
