...
def MoveWhilePress(key: str, fromPos, toPos):
    old_pos = GetCursorPos()
    try:
        windll.user32.SetCursorPos(int(fromPos.x), int(fromPos.y))
        send_key(VK_CODES[key])
        sleep(0.005)
        windll.user32.SetCursorPos(int(toPos.x), int(toPos.y))
        sleep(0.005)
        windll.user32.SetCursorPos(old_pos[0], old_pos[1])
