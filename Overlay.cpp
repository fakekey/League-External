...
#define HCheck(x, m)                                                                                                             \
    if (x != S_OK) {                                                                                                             \
        throw std::runtime_error(Character::Format("DirectX: Failed at %s. Error code: %d\n", m, MAKE_HRESULT(1, _FACDXGI, X))); \
    }
...
void Overlay::Init()
{
    ...
    colors[ImGuiCol_WindowBg] = ImVec4(0.10f, 0.10f, 0.10f, 0.4f);
...
void Overlay::DrawUI(PyGame& state, MemSnapShot& memSnapshot)
{
    ...
    if (!ImGui::Begin("Baker2k", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize)) {
        ImGui::End();
        return;
    }
...
void Overlay::DrawOverlayWindows(PyGame& state)
{
    ...
    ImGui::Begin("##Overlay", &isWOpen,
        ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoCollapse | ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip);
...
{
    ImGui::LabelText("[F8]", "[Show/Hide]");
    if (ImGui::Button("Save all scripts")) {
...
{
    ImGui::LabelText(script->author.c_str(), "[Author]");
    ...
    ImGui::SameLine();
    ImGui::Button("About", sz);
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_Stationary)) {
        ImGui::SetTooltip(script->description.c_str());
    }
    ImGui::Separator();
}
